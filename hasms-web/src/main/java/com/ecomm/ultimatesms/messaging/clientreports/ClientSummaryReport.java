 ...
 */
public class ClientSummaryReport extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ClientSummaryReport() {
        super();
	...
	public void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException  {
		...
		Logger log=LoggerFactory.getLogger(ClientSummaryReport.class);
		...
		if("month".equalsIgnoreCase(displayStatus.trim())){
			...
			for(int i=fromMonthint;i<=toMonthint;i++){String str_key = i+".0"+fromYearint+".0";
			Map smsCountMap = sm.getMonthlyMapSmsCountForAdminSummaryRep(str_key, list);
	...
	if("day".equalsIgnoreCase(displayStatus.trim())){
		...
		for(int i=fromDateint;i<=toDateint;i++){
			...
			Map smsCountMap = sm.getDailyMapSmsCountForAdminSummaryRep(str_key,list);
	...
	rd = request.getRequestDispatcher("reports/clientsummaryreport.jsp");
