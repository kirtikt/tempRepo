...
<%@page import="com.ecomm.pl4sms.persistence.dao.HoldingaccountManager"%>
...
<table border="1px" cellpadding="0" cellspacing="0" width="100%">
	...
	<table border='1' id='flexme1' width='100%'>
		<%
		Long pkey=Long.parseLong(request.getParameter("pKeyHolding"));
		List smsSet=new SmsManager().getSmsListForHolA(pkey);
			Iterator it = smsSet.iterator();
			boolean flag = false;
			out.print("<tr><th>destination</th><th>source</th><th>Status</th><th>SmsId</th><th>Message</th><tr>");
			Helper hp= new Helper();
			while (it.hasNext()) {
				Sms sms = (Sms) it.next();
			   if ((sms.getStatus().trim()).equals("11")){
				out.print("<tr><td>"
						+ sms.getDestination() + "</td><td>" + sms.getSource()
						+ "</td><td >" + hp.getAdminStatusMessage(sms.getStatus(), sms.getSendinginterface().getKannelname(), hp.myTimeIn(sms.getDatetime()).toString())  + "</td><td><a href='/payless4sms/adminreports/DetailSmsIdReport.jsp?smsid="+sms.getSmsid()+"'>" + sms.getSmsid() + "</td><td>"
						+ sms.getMessage().subSequence(0, 30) + "</td></tr>");
			   }
