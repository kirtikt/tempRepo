 ...
 */
public class CreditPerClient extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public CreditPerClient() {
		super();
	...
	public void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException  {
		...
		Logger log=LoggerFactory.getLogger(CreditPerClient.class);
		HttpSession httpSession=request.getSession(false);
		
		String managerid = request.getParameter("clientmanagerId");
		...
		Map map = ha.getBatchesMap(Long.parseLong(managerid), fromDate, toDate);
		
		httpSession.removeAttribute("downloadedMap");
		httpSession.setAttribute("downloadedMap",map);

		request.setAttribute("map",map);
		request.setAttribute("managername", clientname);
		request.setAttribute("clientmanagerId", managerid);
		request.setAttribute("fromDate", from);
		request.setAttribute("toDate", to);
		RequestDispatcher rd = request
				.getRequestDispatcher("adminreports/creditreport.jsp");
