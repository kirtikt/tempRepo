<%@page import="com.ecomm.pl4sms.persistence.dao.MnosmscpropertyManager"%>
<%@page import="org.hibernate.SessionFactory"%>
<%@page import="com.ecomm.pl4sms.persistence.dbutils.HibernateUtil"%>
<%@page import="com.ecomm.ultimatesms.messaging.com.ecomm.pl4sms.persistence.entities.Mnosmscproperty"%>
...
function requiredvalidation(){
	...
	if(document.getElementById("interfacename").value==0 || document.getElementById("concatenation").value==0 || document.getElementById("interfacetype").value==0 || document.getElementById("name").value==""||document.getElementById("sourceadd").value=="" || document.getElementById("deliveryreceipt").value==0 || document.getElementById("smscost").value=="" || document.getElementById("replycost").value=="" ||document.getElementById("kannelname").value==""){
		...
		<tr>
			...
			<td valign="top">
				...
				<div id="login-content">
					<h3 class="heading_text">Edit Interface SMSC proxy</h3>
					<form action="/payless4sms/UpdateSmscInterfaceProperty" id="form"
						...
						<table id="login-table">
							...
							<% 
							String na="n/a";
							String mnoid=request.getParameter("smscinterfaceid");
							
							Long longpropmnoid=null;
							Smscproxyproperty mnoprop=null;
						...
						//	 sessionFactory = HibernateUtil.getSessionFactory();
					     ...
					     //    hibersession=sessionFactory.openSession();
							if(mnoid!=null && mnoid!=""){
								...
								SmscproxypropertyManager mm=new SmscproxypropertyManager();
								
								if(longpropmnoid!=null){
									//mnoprop=(Smscproxyproperty)	hibersession.get(Smscproxyproperty.class, longpropmnoid);
								mnoprop=mm.findById(longpropmnoid); //due to lazy loading
							...
							<tr>
								...
								<option value="<%=mnoprop.getSmscproxy().getPksmscproxyid() %>"><%=mnoprop.getSmscproxy().getName() %></option>
							...
							<td>Interface  Name</td>
								<td><input type="text" name="interfacename"
									class="input required" value="<%if("null".equals(mnoprop.getName())||null==mnoprop.getName()){%><%= na%><% }else{%><%=mnoprop.getName()%><%}%> "/></td>
							</tr>
							<tr>
							...
							<%
						//	Sendinginterface si=mnoprop.getSendinginterfaces().iterator().next();
						    ...
						    while(itmon.hasNext()){
						        ...
						        Mnosmscproperty msp= new MnosmscpropertyManager().getMnosmscproperty(mno,mnoprop);
						        double smscost=0;
						        double replycost=0;
						        String sourcenumber=" ";
						        if(msp!=null){
						    	...
						    	out.println("<td><input type='text' name='"+mno.getName().trim()+"' value='"+smscost+"' /></td>");
						    	 out.println("</tr>");
						    	 out.println("<tr>");
							     out.println("<td>"+mno.getName()+" reply cost</td>");
							     out.println("<td><input type='text' name='r"+mno.getName().trim()+"' value='"+replycost+"'/></td>");
							     out.println("</tr>");
							     out.println("<tr>");
							     out.println("<td>"+mno.getName()+" Source Address</td>");
							     out.println("<td><input type='text' value='n/a' name='sa"+mno.getName().trim()+"' value='"+sourcenumber+"' /></td>");
							     out.println("</tr>");
						    }
							...
							<tr>
								...
								<input type="hidden" name="smscinterfaceid" value="<%=mnoid%> ">
								<input
