		...
		<tr>
			...
			<td valign="top">
				...
				<table width="70%" align="center">
					<tr>
						<td>
							<form name="form1" action="/payless4sms/CreditPerClient"
								method="post">
								<!-- id="formid" name="form" onSubmit="return checkDayMonth();"> -->
								<table>
									<tr>
										<td>Client :</td>
										<td><c:if test="${not empty param.pKeyManager}">
												<input type="hidden" id="clientmanagerId"
													name="clientmanagerId" value="${param.pKeyManager}" />
											</c:if> 
											<c:if test="${not empty requestScope.clientmanagerId}">
												<input type="hidden" id="clientmanagerId"
													name="clientmanagerId" value="${requestScope.clientmanagerId}" />
											</c:if>
											
											 <c:if test="${not empty param.pKeyManagerName}">
												<input type="hidden" id="clientname"
													name="clientname" value="${param.pKeyManagerName}" />
											</c:if> <c:if test="${not empty requestScope.managername}">
												<input type="hidden" id="clientname"
													name="clientname" value="${requestScope.managername}" />
											</c:if> ${param.clientname}</td>
									</tr>
									<tr>
										<td>From :</td>
					...
					<c:if test="${not empty requestScope.fromDate}">
						...
						<tr style="font-size: 14px">
							<td><b>Client : ${requestScope.managername} &nbsp;
									&nbsp; &nbsp; Report from ${requestScope.fromDate} to
